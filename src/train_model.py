...
def getFiles(emotion): 
    ...
    files = gb.glob("facial-dataset/train/%s/*" %emotion)
    random.shuffle(files)
    training = files[:int(len(files))] #get first 100% of file list
    # prediction = files[-int(len(files)*0.33):] #get last 33% of file list
    return training
def makeTrainingSet():
    print ("Traing set creation : Started")
    training_data = []
    training_labels = []
...
def runClassifier():
    training_data, training_labels = makeTrainingSet()
    ...
    print ("Model saved to result/result.xml")
